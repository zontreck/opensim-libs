...
internal class MainClass
{
    public static int Main(string[] args)
    {
        ...
        if (package != null)
        {
            ...
            {
                Console.WriteLine(
                    "The --registry, --path, --cachepath and --addinspath options\ncan't be used when --package is specified.");
                return 1;
            }

            var app = SetupService.GetExtensibleApplication(package);
            if (app == null)
            {
        ...
        else
        {
            ...
            reg = path != null
                ? new AddinRegistry(path, startupPath, addinsPath, databasePath)
                : AddinRegistry.GetGlobalRegistry();
        }

        try
        {
            var setupTool = new SetupTool(reg);
