...
public:
    ...
    inline_    Matrix4x4 operator+(const Matrix4x4& mat) const
    {
        ...
        __m256 ma, mb;
        ma = _mm256_loadu_ps(m[0]);
        mb = _mm256_loadu_ps(mat.m[0]);
        ma = _mm256_add_ps(ma, mb);
        _mm256_storeu_ps(out.m[0], ma);

        ma = _mm256_loadu_ps(m[2]);
        mb = _mm256_loadu_ps(mat.m[2]);
        ma = _mm256_add_ps(ma, mb);
        _mm256_storeu_ps(out.m[2], ma);
    ...
    inline_ Matrix4x4 operator*(const Matrix4x4& mat) const
    {
        ...
        __m128 ma = _mm_set1_ps(m[0][0]);
        __m128 mb = _mm_set1_ps(m[0][1]);
        __m128 mc = _mm_set1_ps(m[0][2]);
        __m128 md = _mm_set1_ps(m[0][3]);

        mt0 = _mm_loadu_ps(mat.m[0]);
        mt1 = _mm_loadu_ps(mat.m[1]);
        mt2 = _mm_loadu_ps(mat.m[2]);
        mt3 = _mm_loadu_ps(mat.m[3]);

        ma = _mm_mul_ps(ma, mt0);
        mb = _mm_mul_ps(mb, mt1);
        mc = _mm_mul_ps(mc, mt2);
        md = _mm_mul_ps(md, mt3);

        ma = _mm_add_ps(ma, mc);
        mb = _mm_add_ps(mb, md);
        ma = _mm_add_ps(ma, mb);
        _mm_storeu_ps(out.m[0], ma);

        ma = _mm_set1_ps(m[1][0]);
        mb = _mm_set1_ps(m[1][1]);
        mc = _mm_set1_ps(m[1][2]);
        md = _mm_set1_ps(m[1][3]);
        ...
        mb = _mm_add_ps(mb, md);
        ma = _mm_add_ps(ma, mb);
        _mm_storeu_ps(out.m[1], ma);

        ma = _mm_set1_ps(m[2][0]);
        mb = _mm_set1_ps(m[2][1]);
        mc = _mm_set1_ps(m[2][2]);
        md = _mm_set1_ps(m[2][3]);
        ...
        mb = _mm_add_ps(mb, md);
        ma = _mm_add_ps(ma, mb);
        _mm_storeu_ps(out.m[2], ma);

        ma = _mm_set1_ps(m[3][0]);
        mb = _mm_set1_ps(m[3][1]);
        mc = _mm_set1_ps(m[3][2]);
        md = _mm_set1_ps(m[3][3]);
