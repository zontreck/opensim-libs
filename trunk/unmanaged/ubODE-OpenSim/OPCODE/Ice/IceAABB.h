		...
		inline_			void		Rotate(const Matrix4x4& mtx, AABB& aabb)	const
									{
										...
										if(vx.x < 0)	aabb.mMin.x += vx.x; else aabb.mMax.x += vx.x;
										if(vx.y < 0)	aabb.mMin.y += vx.y; else aabb.mMax.y += vx.y;
										if(vx.z < 0)	aabb.mMin.z += vx.z; else aabb.mMax.z += vx.z;
										if(vy.x < 0)	aabb.mMin.x += vy.x; else aabb.mMax.x += vy.x;
										if(vy.y < 0)	aabb.mMin.y += vy.y; else aabb.mMax.y += vy.y;
										if(vy.z < 0)	aabb.mMin.z += vy.z; else aabb.mMax.z += vy.z;
										if(vz.x < 0)	aabb.mMin.x += vz.x; else aabb.mMax.x += vz.x;
										if(vz.y < 0)	aabb.mMin.y += vz.y; else aabb.mMax.y += vz.y;
										if(vz.z < 0)	aabb.mMin.z += vz.z; else aabb.mMax.z += vz.z;
									}
