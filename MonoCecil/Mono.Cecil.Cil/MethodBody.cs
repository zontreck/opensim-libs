...
internal class InstructionCollection : Collection<Instruction>
{
    ...
    private void UpdateDebugInformation(Instruction removedInstruction, Instruction existingInstruction)
    {
        ...
        var resolver = new InstructionOffsetResolver(items, removedInstruction, existingInstruction);
    ...
    private void UpdateStateMachineScope(StateMachineScopeDebugInformation debugInfo,
        ref InstructionOffsetResolver resolver)
    {
        resolver.Restart();
    ...
    private void UpdateAsyncMethodBody(AsyncMethodBodyDebugInformation debugInfo,
        ref InstructionOffsetResolver resolver)
    {
        ...
        resolver.Restart();
        for (var i = 0; i < debugInfo.Yields.Count; i++) debugInfo.Yields[i] = resolver.Resolve(debugInfo.Yields[i]);

        resolver.Restart();
        for (var i = 0; i < debugInfo.Resumes.Count; i++) debugInfo.Resumes[i] = resolver.Resolve(debugInfo.Resumes[i]);
    }
    ...
    private struct InstructionOffsetResolver
    {
        ...
        public InstructionOffsetResolver(Instruction[] instructions, Instruction removedInstruction,
        ...
        private InstructionOffset ResolveInstructionOffset(InstructionOffset inputOffset)
        {
            ...
            else
            {
                ...
                var size = LastOffset;
                for (var i = cache_index; i < items.Length; i++)
                {
                    cache_index = i;
                    LastOffset = size;
