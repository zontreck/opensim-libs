...
public abstract class Association
{
	...
	/// </summary>
	protected internal static readonly DateTime UnixEpoch = new(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
    ...
    private static TimeSpan minimumUsefulAssociationLifetime => Protocol.MaximumUserAgentAuthenticationTime;

    internal bool HasUsefulLifeRemaining => timeTillExpiration >= minimumUsefulAssociationLifetime;
    ...
    /// </summary>
    private TimeSpan timeTillExpiration => Expires - DateTime.UtcNow;

    /// <summary>
    ///     The number of seconds until this <see cref="Association" /> expires.
    ///     Never negative (counter runs to zero).
    /// </summary>
    protected internal long SecondsTillExpiration => Math.Max(0, (long)timeTillExpiration.TotalSeconds);
    ...
    public static Association Deserialize(string handle, DateTime expires, byte[] privateData)
    {
        ...
        var remainingLifeLength = expires - DateTime.UtcNow;
        var secret = privateData; // the whole of privateData is the secret key for now.
    ...
    public byte[] SerializePrivateData()
    {
        ...
        var secretKeyCopy = new byte[SecretKey.Length];
    ...
    internal byte[] Sign(IDictionary<string, string> data, IList<string> keysToSign, string keyLookupPrefix)
    {
        ...
        foreach (var field in keysToSign) nvc.Add(field, data[keyLookupPrefix + field]);
    ...
    public override bool Equals(object obj)
    {
        ...
        if (a.Handle != Handle ||
            a.Issued != Issued ||
            a.TotalLifeLength != TotalLifeLength)
            return false;

        if (!Util.ArrayEquals(a.SecretKey, SecretKey))
            return false;
    ...
    public override int GetHashCode()
    {
        ...
        var hbytes = Encoding.ASCII.GetBytes(Handle);
        ...
        long val = 0;
        for (var i = 0; i < hash.Length; i++) val = val ^ hash[i];

        val = val ^ Expires.ToFileTimeUtc();
