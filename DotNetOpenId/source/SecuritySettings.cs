...
public class SecuritySettings
{
    ...
    {
        if (isProvider) MaximumHashBitLength = maximumHashBitLengthOPDefault;
    }
    ...
    /// </remarks>
    public int MinimumHashBitLength { get; set; } = minimumHashBitLengthDefault;
    ...
    /// </remarks>
    public int MaximumHashBitLength { get; set; } = maximumHashBitLengthRPDefault;

    internal bool IsAssociationInPermittedRange(Protocol protocol, string associationType)
    {
        var lengthInBits = HmacShaAssociation.GetSecretLength(protocol, associationType) * 8;
