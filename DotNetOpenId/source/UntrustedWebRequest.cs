...
namespace DotNetOpenId
{
	...
	public static class UntrustedWebRequest
    {
        ...
        private static int maximumBytesToRead = Configuration.MaximumBytesToRead;

        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        private static int maximumRedirections = Configuration.MaximumRedirections;
        ...
        private static UntrustedWebRequestSection Configuration => UntrustedWebRequestSection.Configuration;
        ...
        /// </summary>
        public static ICollection<string> WhitelistHosts { get; } =
            new List<string>(Configuration.WhitelistHosts.KeysAsStrings);
        ...
        /// </summary>
        public static ICollection<Regex> WhitelistHostsRegex { get; } =
            new List<Regex>(Configuration.WhitelistHostsRegex.KeysAsRegexs);

        /// <summary>
        ///     A collection of host name literals that should be rejected even if they
        ///     pass standard security checks.
        /// </summary>
        public static ICollection<string> BlacklistHosts { get; } =
            new List<string>(Configuration.BlacklistHosts.KeysAsStrings);

        /// <summary>
        ///     A collection of host name regular expressions that indicate hosts that should
        ///     be rjected even if they pass standard security checks.
        /// </summary>
        public static ICollection<Regex> BlacklistHostsRegex { get; } =
            new List<Regex>(Configuration.BlacklistHostsRegex.KeysAsRegexs);
        ...
        private static bool isHostWhitelisted(string host)
        {
        ...
        private static bool isHostInList(string host, ICollection<string> stringList, ICollection<Regex> regexList)
        {
            ...
            foreach (var testHost in stringList)
        ...
        private static bool isUriAllowable(Uri uri)
        {
            ...
            Util.Func<string, bool> failsUnlessWhitelisted = reason =>
            {
            ...
            {
                var addressBytes = hostIPAddress.GetAddressBytes();
        ...
        /// </returns>
        private static void readData(HttpWebResponse resp, out byte[] buffer, out int length)
        {
            var bufferSize = resp.ContentLength >= 0 && resp.ContentLength < int.MaxValue
                ? Math.Min(MaximumBytesToRead, (int)resp.ContentLength)
                : MaximumBytesToRead;
            ...
            using (var stream = resp.GetResponseStream())
            {
                ...
                while (dataLength < bufferSize &&
                       (chunkSize = stream.Read(buffer, dataLength, bufferSize - dataLength)) > 0)
                    dataLength += chunkSize;
                length = dataLength;
            }
        }

        private static UntrustedWebResponse getResponse(Uri requestUri, HttpWebResponse resp)
        {
            byte[] data;
            int length;
            ...
            RequestCachePolicy cachePolicy)
        {
            ...
            var originalRequestUri = uri;
            int i;
            for (i = 0; i < MaximumRedirections; i++)
            {
                var response = RequestInternal(uri, body, acceptTypes, requireSsl, false, originalRequestUri,
                    cachePolicy);
            ...
            throw new WebException(string.Format(CultureInfo.CurrentCulture, Strings.TooManyRedirects,
                originalRequestUri));
        }

        private static UntrustedWebResponse RequestInternal(Uri uri, byte[] body, string[] acceptTypes,
            bool requireSsl, bool avoidSendingExpect100Continue, Uri originalRequestUri, RequestCachePolicy cachePolicy)
        {
            ...
            if (requireSsl && !string.Equals(uri.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase))
                throw new OpenIdException(string.Format(CultureInfo.CurrentCulture,
                    Strings.InsecureWebRequestWithSslRequired, uri));

            // mock the request if a hosting unit test has configured it.
            if (MockRequests != null) return MockRequests(uri, body, acceptTypes);
