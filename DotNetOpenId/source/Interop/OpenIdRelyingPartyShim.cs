...
[ComVisible(true)]
[Obsolete("This class acts as a COM Server and should not be called directly from .NET code.", true)]
...
public class OpenIdRelyingPartyShim : IOpenIdRelyingParty
{
    ...
    public string CreateRequest(string userSuppliedIdentifier, string realm, string returnToUrl)
    {
        ...
        var response = (Response)rp.CreateRequest(userSuppliedIdentifier, realm, new Uri(returnToUrl))
            .RedirectingResponse;
    ...
    {
        var uri = new Uri(url);
        var rp = new OpenIdRelyingParty(null, uri, HttpUtility.ParseQueryString(uri.Query));
        if (rp.Response != null) return new AuthenticationResponseShim(rp.Response);
