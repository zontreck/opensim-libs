...
public class OpenIdException : Exception, IEncodable
{
    ...
    internal bool HasReturnTo => Query == null ? false : Query.ContainsKey(Protocol.openid.return_to);
    ...
    private bool IsDirectMessage
    {
        get
        {
            Debug.Assert(Query != null,
                "An OpenId exception should always be provided with the query if it is to be encoded for transmittal to the RP.");

            if (Query != null)
            {
                var mode = Util.GetOptionalArg(Query, Protocol.openid.mode);
                if (mode != null)
    ...
    EncodingType IEncodable.EncodingType
    {
        get
        {
            ...
            Debug.Fail(
                "Somehow we cannot tell whether this is a direct message or indirect message.  Did we construct an exception without a Query parameter?");
