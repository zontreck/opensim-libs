...
internal class KeyValueFormEncoding : IProtocolMessageEncoding
{
    ...
    public KeyValueFormConformanceLevel ConformanceLevel { get; }
    ...
    {
        using (var reader = new StreamReader(data, textEncoding))
        {
    ...
    public byte[] GetBytes(IDictionary<string, string> dictionary, IList<string> keyOrder)
    {
        ...
        using (var sw = new StreamWriter(ms, textEncoding))
        {
            sw.NewLine = newLineCharacters;
            foreach (var keyInOrder in keyOrder)
            {
                var key = keyInOrder.Trim();
                var value = dictionary[key].Trim();
