...
public enum AuthenticationStatus
{
	...
	///     </para>
	///     <para>
	///         Casting the <see cref="IAuthenticationResponse" /> to a
...
internal class AuthenticationResponse : IAuthenticationResponse, ISetupRequiredAuthenticationResponse
{
    ...
    /// </summary>
    private readonly IDictionary<string, string> signedArguments;

    internal AuthenticationResponse(AuthenticationStatus status, ServiceEndpoint provider,
        IDictionary<string, string> query)
    {
        ...
        if (query.TryGetValue(Provider.Protocol.openid.signed, out signed))
            foreach (var fieldNoPrefix in signed.Split(','))
            {
                var fieldWithPrefix = Provider.Protocol.openid.Prefix + fieldNoPrefix;
                string val;
    ...
    /// </summary>
    internal ServiceEndpoint Provider { get; }

    /// <summary>
    ///     Gets the set of arguments that the Provider included as extensions.
    /// </summary>
    public ExtensionArgumentsManager IncomingExtensions { get; }

    internal Uri ReturnTo => new(Util.GetRequiredArg(signedArguments, Provider.Protocol.openid.return_to));
    ...
    {
        [DebuggerStepThrough] get => Provider.FriendlyIdentifierForDisplay;
    }
    ...
    public IExtensionResponse GetExtension(Type extensionType)
    {
        ...
        if (!typeof(IExtensionResponse).IsAssignableFrom(extensionType))
    ...
    {
        if (extensionType == null) throw new ArgumentNullException("extensionType");
        if (!typeof(IClientScriptExtensionResponse).IsAssignableFrom(extensionType))
            throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
    ...
    private bool getExtension(IExtensionResponse extension)
    {
        var fields = IncomingExtensions.GetExtensionArguments(extension.TypeUri);
        if (fields != null)
            // The extension was found using the preferred TypeUri.
        ...
        OpenIdRelyingParty relyingParty, Uri requestUrl, bool verifySignature)
    {
        ...
        var requestUrlQuery = Util.NameValueCollectionToDictionary(
            HttpUtility.ParseQueryString(requestUrl.Query));
        var token = Util.GetOptionalArg(requestUrlQuery, Token.TokenKey);
        if (token != null) tokenEndpoint = Token.Deserialize(token, relyingParty.Store).Endpoint;
        ...
        if (mode.Equals(protocol.Args.Mode.setup_needed, StringComparison.Ordinal))
            return new AuthenticationResponse(AuthenticationStatus.SetupRequired, tokenEndpoint, query);

        if (mode.Equals(protocol.Args.Mode.error, StringComparison.Ordinal))
            throw new OpenIdException(string.Format(CultureInfo.CurrentCulture,
                "The provider returned an error: {0}", query[protocol.openid.error]));

        if (mode.Equals(protocol.Args.Mode.id_res, StringComparison.Ordinal))
        {
            ...
            else
            {
                ...
                var friendlyIdentifier = tokenEndpoint != null ? tokenEndpoint.UserSuppliedIdentifier : null;
    ...
    private static AuthenticationResponse parseIdResResponse(IDictionary<string, string> query,
        ...
        OpenIdRelyingParty relyingParty, Uri requestUrl, bool verifyMessageSignature)
    {
        ...
        var unverifiedEndpoint = responseEndpoint ?? tokenEndpoint;
        if (unverifiedEndpoint.Protocol.Version.Major < 2)
        {
            var user_setup_url = Util.GetOptionalArg(query, unverifiedEndpoint.Protocol.openid.user_setup_url);
    ...
    /// </remarks>
    private static void verifyReturnTo(IDictionary<string, string> query, ServiceEndpoint endpoint, Uri requestUrl)
    {
        ...
        var return_to = new Uri(Util.GetRequiredArg(query, endpoint.Protocol.openid.return_to));
        ...
        var returnToArgs = HttpUtility.ParseQueryString(return_to.Query);
        var requestArgs = HttpUtility.ParseQueryString(requestUrl.Query);
    ...
    /// </remarks>
    private static void verifyDiscoveredInfoMatchesAssertedInfo(OpenIdRelyingParty relyingParty,
        IDictionary<string, string> query,
        ServiceEndpoint tokenEndpoint, ServiceEndpoint responseEndpoint)
    {
        ...
        {
            Debug.Assert(tokenEndpoint != null,
                "Our OpenID 1.x implementation requires an RP token.  And this should have been verified by our caller.");
        ...
        else
        {
            ...
            {
                // or directed identity is in effect
                ...
                var discoveredEndpoints = new List<ServiceEndpoint>(claimedIdentifier.Discover());
    ...
    private static void verifyNonceUnused(IDictionary<string, string> query, ServiceEndpoint endpoint,
    ...
    private static void verifySignature(OpenIdRelyingParty relyingParty, IDictionary<string, string> query,
        ServiceEndpoint endpoint)
    {
        var signed = Util.GetRequiredArg(query, endpoint.Protocol.openid.signed);
        var signedFields = signed.Split(',');
        ...
        var assoc_handle = Util.GetRequiredArg(query, endpoint.Protocol.openid.assoc_handle);
        var assoc = relyingParty.Store != null
            ? relyingParty.Store.GetAssociation(endpoint.ProviderEndpoint, assoc_handle)
            : null;
    ...
    /// </summary>
    private static void verifyFieldsAreSigned(string[] fieldsThatAreSigned, params string[] fieldsThatShouldBeSigned)
    {
        Debug.Assert(fieldsThatAreSigned != null);
        Debug.Assert(fieldsThatShouldBeSigned != null);
        foreach (var field in fieldsThatShouldBeSigned)
    ...
    private static void verifySignatureByAssociation(IDictionary<string, string> query, Protocol protocol,
        string[] signedFields, Association assoc)
    {
        var sig = Util.GetRequiredArg(query, protocol.openid.sig);

        var v_sig = Convert.ToBase64String(assoc.Sign(query, signedFields, protocol.openid.Prefix));
    ...
    private static void verifySignatureByProvider(OpenIdRelyingParty relyingParty, IDictionary<string, string> query,
        ServiceEndpoint provider)
    {
        ...
        if (request.Response.InvalidatedAssociationHandle != null && relyingParty.Store != null)
            relyingParty.Store.RemoveAssociation(provider.ProviderEndpoint,
                request.Response.InvalidatedAssociationHandle);
        if (!request.Response.IsAuthenticationValid)
            throw new OpenIdException(Strings.InvalidSignature);
    }

    private static IDictionary<string, string> cleanQueryForCallbackArguments(IDictionary<string, string> query)
    {
        var dictionary = new Dictionary<string, string>();
        foreach (var pair in query)
        {
            // Disallow lookup of any openid parameters.
            if (pair.Key.StartsWith("openid.", StringComparison.OrdinalIgnoreCase)) continue;
