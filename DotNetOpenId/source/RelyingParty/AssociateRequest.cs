...
internal class AssociateRequest : DirectRequest
{
    ...
    private AssociateRequest(OpenIdRelyingParty relyingParty, ServiceEndpoint provider,
    ...
    public AssociateResponse Response
    {
        get
        {
            if (response == null)
                ...
                {
                    if (ex.Query != null) response = new AssociateResponse(RelyingParty, Provider, ex.Query, DH);
    ...
    public static AssociateRequest Create(OpenIdRelyingParty relyingParty, ServiceEndpoint provider)
    {
        ...
        Logger.Warn(
            "Security requirements and protocol combination knock out all possible association types.  Dumb mode forced.");
        return null;
    }

    public static AssociateRequest Create(OpenIdRelyingParty relyingParty, ServiceEndpoint provider, string assoc_type,
        string session_type, bool allowNoSession)
    {
        if (relyingParty == null) throw new ArgumentNullException("relyingParty");
        if (provider == null) throw new ArgumentNullException("provider");
        ...
        else
        {
            ...
            var dhPublic = dh.CreateKeyExchange();
            var cpub = DiffieHellmanUtil.UnsignedToBase64(dhPublic);

            args.Add(protocol.openid.session_type, session_type);
            args.Add(protocol.openid.dh_consumer_public, cpub);

            var dhps = dh.ExportParameters(true);
