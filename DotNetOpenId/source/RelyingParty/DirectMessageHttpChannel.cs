...
internal class DirectMessageHttpChannel : IDirectMessageChannel
{
    ...
    public IDictionary<string, string> SendDirectMessageAndGetResponse(ServiceEndpoint provider,
        IDictionary<string, string> fields)
    {
        if (provider == null) throw new ArgumentNullException("provider");
        if (fields == null) throw new ArgumentNullException("fields");

        var body = ProtocolMessages.Http.GetBytes(fields);
        ...
        try
        {
            ...
            {
                var errorStream = new StreamReader(resp.ResponseStream).ReadToEnd();
