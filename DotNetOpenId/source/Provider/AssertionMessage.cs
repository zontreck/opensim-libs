...
internal static class AssertionMessage
{
        ...
        OpenIdProvider provider, Identifier localIdentifier, Identifier claimedIdentifier)
    {
        ...
        Debug.Assert(!message.Signed.Contains(protocol.openidnp.mode),
            "openid.mode must not be signed because it changes in check_authentication requests.");
    ...
    public static EncodableResponse CreateAssertion(CheckIdRequest request)
    {
        ...
        var response = EncodableResponse.PrepareIndirectMessage(
            request.Protocol, request.ReturnTo, request.AssociationHandle);
        if (request.IsAuthenticated.Value)
        {
            CreatePositiveAssertion(response, request.Provider,
        ...
        {
            CreateNegativeAssertion(response, request);
        ...
        Realm relyingParty, Identifier claimedIdentifier, Identifier localIdentifier)
    {
        ...
        var message = EncodableResponse.PrepareIndirectMessage(
