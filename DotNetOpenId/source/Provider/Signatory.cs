...
internal class Signatory
{
	...
	/// </summary>
	private static readonly TimeSpan smartAssociationLifetime = TimeSpan.FromDays(14);

	/// <summary>
	///     The duration a secret key used for signing dumb client requests will be good for.
	/// </summary>
	private static readonly TimeSpan dumbSecretLifetime = TimeSpan.FromMinutes(5);

	/// <summary>
	///     The store for shared secrets.
	/// </summary>
	private readonly IProviderAssociationStore store;
    ...
    {
        Association assoc;
        var assoc_handle = response.PreferredAssociationHandle;
        ...
        if (!string.IsNullOrEmpty(assoc_handle))
        {
            ...
            {
                Logger.WarnFormat(
                    "No associaton found with assoc_handle {0}. Setting invalidate_handle and creating new Association.",
                    assoc_handle);
    ...
    {
        var assoc = GetAssociation(assoc_handle, AssociationRelyingPartyType.Dumb);
        if (assoc == null)
        {
        ...
        var expected_sig = Convert.ToBase64String(assoc.Sign(signed_pairs, signedKeyOrder));
    ...
    public virtual Association CreateAssociation(AssociationRelyingPartyType associationType, OpenIdProvider provider)
    {
        ...
        var secretLength = HmacShaAssociation.GetSecretLength(associationProtocol, assoc_type);

        var generator = new RNGCryptoServiceProvider();
        var secret = new byte[secretLength];
        var uniq_bytes = new byte[4];
        string uniq;
        string handle;
        ...
        var seconds = DateTime.UtcNow.Subtract(Association.UnixEpoch).TotalSeconds;

        handle = "{{" + assoc_type + "}{" + seconds + "}{" + uniq + "}";

        var lifeSpan = associationType == AssociationRelyingPartyType.Dumb
            ? dumbSecretLifetime
    ...
    public virtual Association GetAssociation(string assoc_handle, AssociationRelyingPartyType associationType)
    {
        ...
        var assoc = store.GetAssociation(associationType, assoc_handle);
