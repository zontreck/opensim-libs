...
{
    private readonly string Mode = "fetch_request";

    private readonly List<AttributeRequest> attributesRequested = new();

    /// <summary>
    ///     Enumerates the attributes whose values are requested by the Relying Party.
    /// </summary>
    public IEnumerable<AttributeRequest> Attributes => attributesRequested;
    ...
    IDictionary<string, string> IExtensionRequest.Serialize(IAuthenticationRequest authenticationRequest)
    {
        ...
        List<string> requiredAliases = new(), optionalAliases = new();
    ...
    bool IExtensionRequest.Deserialize(IDictionary<string, string> fields, IRequest request, string typeUri)
    {
        ...
        {
            Logger.Error(
                "Attribute Exchange extension did not provide any aliases in the if_available or required lists.");
            return false;
        }
    ...
    private static List<string> parseAliasList(string aliasList)
    {
