...
public sealed class PolicyResponse : IExtensionResponse
{
    ...
    /// </summary>
    /// <remarks>
    ///     If no policies were met though the OP wishes to convey other information in the response, this parameter MUST be
    ///     included with the value of "none".
    /// </remarks>
    public IList<string> ActualPolicies { get; }

    /// <summary>
    ///     Optional. The most recent timestamp when the End User has actively authenticated to the OP in a manner fitting the
    ///     asserted policies.
    /// </summary>
    /// <remarks>
    ///     If the RP's request included the "openid.max_auth_age" parameter then the OP MUST include "openid.auth_time" in its
    ///     response. If "openid.max_auth_age" was not requested, the OP MAY choose to include "openid.auth_time" in its
    ///     response.
    /// </remarks>
    public DateTime? AuthenticationTimeUtc
    {
        get => authenticationTimeUtc;
        set
        {
    ...
    /// <summary>
    ///     Optional. The Assurance Level as defined by the National Institute of Standards and Technology (NIST) in Special
    ///     Publication 800-63 (Burr, W., Dodson, D., and W. Polk, Ed., “Electronic Authentication Guideline,” April 2006.)
    ///     [NIST_SP800‑63] corresponding to the authentication method and policies employed by the OP when authenticating the
    ///     End User.
    /// </summary>
    /// <remarks>
    ///     See PAPE spec Appendix A.1.2 (NIST Assurance Levels) for high-level example classifications of authentication
    ///     methods within the defined levels.
    /// </remarks>
    public NistAssuranceLevel? NistAssuranceLevel
    {
        ...
        {
            if (value != null)
                AssuranceLevels[Constants.AuthenticationLevels.NistTypeUri] =
    ...
    /// </remarks>
    public IDictionary<string, string> AssuranceLevels { get; }
    ...
    {
        var other = obj as PolicyResponse;
        ...
        foreach (var policy in ActualPolicies)
    ...
    IDictionary<string, string> IExtensionResponse.Serialize(IRequest authenticationRequest)
    {
        ...
        if (AuthenticationTimeUtc.HasValue)
            fields.Add(Constants.ResponseParameters.AuthTime,
                AuthenticationTimeUtc.Value.ToUniversalTime()
                    .ToString(PermissibleDateTimeFormats[0], CultureInfo.InvariantCulture));

        if (AssuranceLevels.Count > 0)
        {
            var aliases = new AliasManager();
        ...
        string typeUri)
    {
        ...
        var actualPolicies = fields[Constants.ResponseParameters.AuthPolicies].Split(' ');
        foreach (var policy in actualPolicies)
        ...
        if (fields.TryGetValue(Constants.ResponseParameters.AuthTime, out authTime))
        {
            ...
            if (DateTime.TryParse(authTime, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal,
                    out authDateTime) &&
        ...
        var authLevelAliases = PolicyRequest.FindIncomingAliases(fields);
        foreach (var authLevelAlias in authLevelAliases.Aliases)
        {
            string authValue;
            if (fields.TryGetValue(Constants.ResponseParameters.AuthLevelAliasPrefix + authLevelAlias, out authValue))
            {
                var authLevelType = authLevelAliases.ResolveAlias(authLevelAlias);
