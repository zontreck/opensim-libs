...
internal class AliasManager
{
    ...
    /// </summary>
    private readonly Dictionary<string, string> aliasToTypeUriMap = new();

    /// <summary>
    ///     Tracks extension Type URIs and aliases assigned to them.
    /// </summary>
    private readonly Dictionary<string, string> typeUriToAliasMap = new();

    public IEnumerable<string> Aliases => aliasToTypeUriMap.Keys;
    ...
    /// <param name="preferredTypeUriToAliases">
    ///     An optional dictionary of URI/alias pairs that suggest preferred aliases to use
    ///     if available for certain type URIs.
    /// </param>
    ...
    {
        var typeUri = TryResolveAlias(alias);
        if (typeUri == null)
    ...
    private string assignNewAlias(string typeUri)
    {
        ...
        var alias = string.Format(CultureInfo.InvariantCulture, aliasFormat, typeUriToAliasMap.Count + 1);
        typeUriToAliasMap.Add(typeUri, alias);
        aliasToTypeUriMap.Add(alias, typeUri);
        return alias;
    }
