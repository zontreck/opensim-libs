...
{
    private static readonly Regex birthDateValidator = new(@"^\d\d\d\d-\d\d-\d\d$");
    ...
    public DateTime? BirthDate
    {
        ...
        set
        {
            ...
            if (value.HasValue)
                birthDateRaw = value.Value.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            else
                birthDateRaw = null;
        }
    }
    ...
    /// <summary>
    ///     The string value of the birthdate.  Useful for getting/setting when month and/or day information is not included in
    ///     the date
    ...
    public string BirthDateRaw
    {
        ...
        set
        {
            if (value != null)
            {
                ...
                {
                    Logger.InfoFormat(
                        "Simple Registration birthdate '{0}' could not be parsed into a DateTime and may not include month and/or day information.  Setting BirthDate property to null.",
                        value);
                    birthDate = null;
                }
            }
    ...
    public CultureInfo Culture
    {
        ...
        set
        {
            ...
            var indexOfHyphen = value != null ? value.Name.IndexOf('-') : -1;
    ...
    {
        var other = obj as ClaimsResponse;
    ...
    string IExtension.TypeUri => typeUriToUse;
    ...
    IDictionary<string, string> IExtensionResponse.Serialize(IRequest authenticationRequest)
    {
        ...
        if (BirthDateRaw != null) fields.Add(Constants.dob, BirthDateRaw);
        if (!string.IsNullOrEmpty(Country)) fields.Add(Constants.country, Country);
        if (Email != null) fields.Add(Constants.email, Email);
        if (!string.IsNullOrEmpty(FullName)) fields.Add(Constants.fullname, FullName);
        if (Gender != null)
        {
        ...
        if (!string.IsNullOrEmpty(Language)) fields.Add(Constants.language, Language);
        if (!string.IsNullOrEmpty(Nickname)) fields.Add(Constants.nickname, Nickname);
        if (!string.IsNullOrEmpty(PostalCode)) fields.Add(Constants.postcode, PostalCode);
        if (!string.IsNullOrEmpty(TimeZone)) fields.Add(Constants.timezone, TimeZone);
        return fields;
    }
        ...
        if (sreg.TryGetValue(Constants.gender, out genderString))
            switch (genderString)
            {
                case Constants.Genders.Male:
                    Gender = SimpleRegistration.Gender.Male;
                    break;
                case Constants.Genders.Female:
                    Gender = SimpleRegistration.Gender.Female;
                    break;
            }
    ...
    string IClientScriptExtensionResponse.InitializeJavaScriptData(IDictionary<string, string> sreg,
        IAuthenticationResponse response, string typeUri)
    {
        ...
        if (sreg.TryGetValue(Constants.email, out email)) builder.Append(createAddFieldJS(Constants.email, email));
        if (sreg.TryGetValue(Constants.fullname, out fullName))
            builder.Append(createAddFieldJS(Constants.fullname, fullName));
        if (sreg.TryGetValue(Constants.dob, out dob)) builder.Append(createAddFieldJS(Constants.dob, dob));
        if (sreg.TryGetValue(Constants.gender, out genderString))
