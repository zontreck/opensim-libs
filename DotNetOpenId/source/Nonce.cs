...
public class Nonce
{
    private const uint UniqueFragmentLength = 8;
    ...
    private readonly bool remoteServerOrigin;
    ...
    internal Nonce(string code, bool remoteServerOrigin)
    {
        ...
        Code = code;
        var indexOfDateEnd = code.IndexOf("Z", StringComparison.Ordinal);
    ...
    {
        Code = creation.ToUniversalTime().ToString(PermissibleDateTimeFormats[0], CultureInfo.InvariantCulture) +
               uniqueFragment;
    ...
    /// </summary>
    public DateTime ExpirationDate => CreationDate + maximumLifetime;

    private static string generateUniqueFragment()
    {
        var nonce = new char[UniqueFragmentLength];
        for (var i = 0; i < nonce.Length; i++) nonce[i] = allowedCharacters[generator.Next(allowedCharacters.Length)];
