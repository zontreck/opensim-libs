...
internal class DiffieHellmanUtil
{
    private static readonly DHSha[] DiffieHellmanSessionTypes =
    {
        new(new SHA512Managed(), protocol => protocol.Args.SessionType.DH_SHA512),
        new(new SHA384Managed(), protocol => protocol.Args.SessionType.DH_SHA384),
        new(new SHA256Managed(), protocol => protocol.Args.SessionType.DH_SHA256),
        new(new SHA1Managed(), protocol => protocol.Args.SessionType.DH_SHA1)
    };
    ...
    {
        foreach (var dhsha in DiffieHellmanSessionTypes)
    ...
    {
        var dhShared = dh.DecryptKeyExchange(keyEx);
        var shaDhShared = hasher.ComputeHash(ensurePositive(dhShared));
        ...
        var secret = new byte[encMacKey.Length];
        for (var i = 0; i < encMacKey.Length; i++) secret[i] = (byte)(encMacKey[i] ^ shaDhShared[i]);
        return secret;
    }
    ...
    private static byte[] ensurePositive(byte[] inputBytes)
    {
        if (inputBytes.Length == 0)
            throw new ArgumentException("Invalid input passed to EnsurePositive. Array must have something in it.",
                "inputBytes");

        int i = inputBytes[0];
        if (i > 127)
        {
            var temp = new byte[inputBytes.Length + 1];
            temp[0] = 0;
    ...
    private class DHSha
    {
        ...
        internal readonly Util.Func<Protocol, string> GetName;
