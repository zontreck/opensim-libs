...
public sealed class UriIdentifier : Identifier
{
    ...
    private static bool TryCanonicalize(string uri, out Uri canonicalUri, bool forceHttpsDefaultScheme,
        out bool schemePrepended)
    {
        canonicalUri = null;
    ...
    internal override IEnumerable<ServiceEndpoint> Discover()
    {
        ...
        var yadisResult = Yadis.Yadis.Discover(this, IsDiscoverySecureEndToEnd);
        if (yadisResult != null)
            if (yadisResult.IsXrds)
            {
                ...
                if (IsDiscoverySecureEndToEnd) xrdsEndpoints = Util.Where(xrdsEndpoints, se => se.IsSecure);
    ...
    internal override bool TryRequireSsl(out Identifier secureIdentifier)
    {
        ...
        {
            secureIdentifier = new UriIdentifier(Uri, true);
            return true;
        }
        ...
        {
            var newIdentifierUri = new UriBuilder(Uri);
            newIdentifierUri.Scheme = Uri.UriSchemeHttps;
            if (newIdentifierUri.Port == 80) newIdentifierUri.Port = 443;
    ...
    {
        var other = obj as UriIdentifier;
