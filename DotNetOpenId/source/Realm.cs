...
public class Realm
{
    ...
    private static readonly string[] _top_level_domains =
    {
        "com", "edu", "gov", "int", "mil", "net", "org", "biz", "info", "name", "museum", "coop", "aero", "ac", "ad",
        "ae",
        "af", "ag", "ai", "al", "am", "an", "ao", "aq", "ar", "as", "at", "au", "aw", "az", "ba", "bb", "bd", "be",
        "bf", "bg", "bh", "bi", "bj",
        ...
        "re", "ro", "ru", "rw", "sa",
        "sb", "sc", "sd", "se", "sg", "sh", "si", "sj", "sk", "sl", "sm", "sn", "so", "sr", "st", "sv", "sy", "sz",
        "tc", "td", "tf", "tg", "th",
    ...
    public Realm(string realmUrl)
    {
        ...
        NoWildcardUri = new Uri(Regex.Replace(realmUrl, wildcardDetectionPattern, m => m.Groups[1].Value));
        if (!NoWildcardUri.Scheme.Equals("http", StringComparison.OrdinalIgnoreCase) &&
            !NoWildcardUri.Scheme.Equals("https", StringComparison.OrdinalIgnoreCase))
            throw new UriFormatException(string.Format(CultureInfo.CurrentCulture,
                Strings.InvalidScheme, NoWildcardUri.Scheme));
    }
    ...
    /// </summary>
    public bool DomainWildcard { get; }

    /// <summary>
    ///     Gets the host component of this instance.
    /// </summary>
    public string Host => NoWildcardUri.Host;

    /// <summary>
    ///     Gets the scheme name for this URI.
    /// </summary>
    public string Scheme => NoWildcardUri.Scheme;

    /// <summary>
    ///     Gets the port number of this URI.
    /// </summary>
    public int Port => NoWildcardUri.Port;

    /// <summary>
    ///     Gets the absolute path of the URI.
    /// </summary>
    public string AbsolutePath => NoWildcardUri.AbsolutePath;

    /// <summary>
    ///     Gets the System.Uri.AbsolutePath and System.Uri.Query properties separated
    ///     by a question mark (?).
    /// </summary>
    public string PathAndQuery => NoWildcardUri.PathAndQuery;
    ...
    internal Uri UriWithWildcardChangedToWww
    {
        get
        {
            if (DomainWildcard)
            {
                var builder = new UriBuilder(NoWildcardUri);
    ...
    internal bool IsSane
    {
        get
        {
            ...
            var tld = host_parts[host_parts.Length - 1];
    ...
    internal bool Contains(Uri url)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < host_parts.Length; i++)
            {
                var hostPart = host_parts[host_parts.Length - 1 - i];
                var urlPart = url_parts[url_parts.Length - 1 - i];
                if (!string.Equals(hostPart, urlPart, StringComparison.OrdinalIgnoreCase)) return false;
            }
        }
        ...
        var path_len = PathAndQuery.Length;
        var url_prefix = url.PathAndQuery.Substring(0, path_len);
    ...
    internal IEnumerable<RelyingPartyReceivingEndpoint> Discover(bool allowRedirects)
    {
        // Attempt YADIS discovery
        var yadisResult = Yadis.Yadis.Discover(UriWithWildcardChangedToWww, false);
        if (yadisResult != null)
        {
            ...
            if (yadisResult.IsXrds)
                try
                {
                    var xrds = new XrdsDocument(yadisResult.ResponseText);
    ...
    public override bool Equals(object obj)
    {
        ...
        return NoWildcardUri.Equals(other.NoWildcardUri) && DomainWildcard == other.DomainWildcard;
    }
    ...
    {
        return NoWildcardUri.GetHashCode() + (DomainWildcard ? 1 : 0);
    }
    ...
    public override string ToString()
    {
        ...
        return NoWildcardUri.AbsoluteUri;
    }
}