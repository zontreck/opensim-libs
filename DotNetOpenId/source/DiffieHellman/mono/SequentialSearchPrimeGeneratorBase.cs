...
internal class SequentialSearchPrimeGeneratorBase : PrimeGeneratorBase
{
    ...
    {
        var ret = BigInteger.genRandom(bits);
        ret.setBit(0);
        return ret;
    }
    ...
    public virtual BigInteger GenerateNewPrime(int bits, object Context)
    {
        ...
        //
        var curVal = GenerateSearchBase(bits, Context);

        const uint primeProd1 = 3u * 5u * 7u * 11u * 13u * 17u * 19u * 23u * 29u;

        var pMod1 = curVal % primeProd1;

        var DivisionBound = TrialDivisionBounds;
        var SmallPrimes = BigInteger.smallPrimes;
        var PostTrialDivisionTest = PrimalityTest;
        //
        ...
        while (true)
        {
            ...
            //
            for (var p = 9; p < SmallPrimes.Length && SmallPrimes[p] <= DivisionBound; p++)
